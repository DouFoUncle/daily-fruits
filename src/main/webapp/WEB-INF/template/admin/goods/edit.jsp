    ...
    <form id="myform" action="" method="post" class="layui-form-pane layui-form">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="goodsName" lay-verify="required" placeholder="请输入商品名称" autocomplete="off" class="layui-input" value="${editInfo.goodsName}">
            </div>
                ...
                <select name="goodsType">
                    ...
                    <c:forEach var="item" items="${typeList}">
                        <option value="${item.id}|${item.typeName}"
                            <c:if test="${item.typeName eq editInfo.goodsType}">selected</c:if> >${item.typeName}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-inline">
                <input type="text" name="goodsKeyword" lay-verify="required" placeholder="例如：娃娃菜,皇帝菜(多个用;英文分号隔开)" autocomplete="off" class="layui-input" value="${editInfo.goodsKeyword}">
            </div>

            <label class="layui-form-label" style="font-size: 14px">商品售价</label>
            <div class="layui-input-inline">
                <input type="text" name="goodsPriceShow" lay-verify="required" placeholder="请输入商品售价" autocomplete="off" class="layui-input" value="${editInfo.goodsPrice / 100}">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label" style="font-size: 14px">进货价</label>
            <div class="layui-input-inline">
                <input type="text" name="goodsCostPriceShow" lay-verify="required" placeholder="请输入商品进货价" autocomplete="off" class="layui-input" value="${editInfo.goodsCostPrice / 100}">
            </div>

            <label class="layui-form-label" style="font-size: 14px">商品库存</label>
            <div class="layui-input-inline">
                <input type="text" name="goodsStock" lay-verify="required" placeholder="请输入商品库存" autocomplete="off" class="layui-input" value="${editInfo.goodsStock}">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label" style="font-size: 14px">商品首图</label>
            <div class="layui-input-inline">
                <input type="text" name="goodsHeadImg" lay-verify="required"
                       placeholder="请填入商品首图的名字" autocomplete="off"
                       class="layui-input" value="${editInfo.goodsHeadImg}">
            </div>

            <label class="layui-form-label" style="font-size: 14px">展示图名</label>
            <div class="layui-input-inline">
                <input type="text" name="goodsImg" lay-verify="required"
                       placeholder="多个用;分割例如：1.png;2.png" autocomplete="off"
                       class="layui-input" value="${editInfo.goodsImg}">
            </div>
        </div>
        <div class="layui-form-item">
            <label class="layui-form-label" style="font-size: 14px">商品规格</label>
            <div class="layui-input-inline">
                <input type="text" name="goodsNorms" lay-verify="required"
                       placeholder="请填写商品规格" autocomplete="off"
                       class="layui-input" value="${editInfo.goodsNorms}">
            </div>
        </div>
        <div class="layui-form-item layui-form-text">
            <label class="layui-form-label">商品描述</label>
            <div class="layui-input-block">
                <textarea placeholder="请输入商品描述" class="layui-textarea" name="goodsReadme"
                          style="height: 120px; resize: none">${editInfo.goodsReadme}</textarea>
            </div>
        </div>
        <div>
