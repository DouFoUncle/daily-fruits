...
<div style="padding: 15px;">
    <form id="myform" action="" method="post" class="layui-form-pane layui-form">
        ...
        <input type="hidden" name="provinceOldNum" value="${addressInfo.provinceNum}">
        <input type="hidden" name="cityOldNum" value="${addressInfo.cityNum}">
        <input type="hidden" name="countyOldNum" value="${addressInfo.countyNum}">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="addressName"
                       ...
                       autocomplete="off" class="layui-input" value="${addressInfo.addressName}">
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="phone"
                       ...
                       autocomplete="off" class="layui-input" value="${addressInfo.phone}">
            </div>
        </div>
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="locationAddress"
                       ...
                       autocomplete="off" class="layui-input" value="${addressInfo.locationAddress}">
            </div>
        </div>
        <div>
...
<script type="text/javascript">
    layui.use(['upload', 'form'], function () {
        ...
        // 保存原来的编号
        let provinceOldNum = $("input[name='provinceOldNum']").val();
        let cityOldNum = $("input[name='cityOldNum']").val();
        let countyOldNum = $("input[name='countyOldNum']").val();
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    for (let x in provinceJsonInfo) {
                        if(provinceJsonInfo[x].code == provinceOldNum) {
                            let option = new Option(provinceJsonInfo[x].name, provinceJsonInfo[x].code+"|"+provinceJsonInfo[x].name);
                            option.selected = "selected"
                            provinceCode = provinceOldNum;
                    ...
                    for (let x in cityJsonInfo) {
                        if(cityJsonInfo[x].code.substring(0, 2) == provinceCode.substring(0, 2)) {
                            if (cityJsonInfo[x].code == cityOldNum) {
                                let option = new Option(cityJsonInfo[x].name, cityJsonInfo[x].code + "|" + cityJsonInfo[x].name);
                                option.selected = "selected"
                                cityCode = cityOldNum;
                    ...
                    for (let x in countyJsonInfo) {
                        if(countyJsonInfo[x].code.substring(0, 4) == cityCode.substring(0, 4)) {
                            if (countyJsonInfo[x].code == countyOldNum) {
                                let option = new Option(countyJsonInfo[x].name, countyJsonInfo[x].code + "|" + countyJsonInfo[x].name);
                                option.selected = "selected"
            ...
            if(!provinceCode) {
                ...
                for (let x in cityJsonInfo) {
                    if(cityJsonInfo[x].code.substring(0, 2) == data.value.substring(0, 2)) {
                        if(cityJsonInfo[x].code == cityOldNum) {
                            console.log(cityJsonInfo[x].code)
                            let option = new Option(cityJsonInfo[x].name, cityJsonInfo[x].code + "|" + cityJsonInfo[x].name);
                            option.selected = "selected";
                            citySelect.append(option);
                        } else {
